export const langTag = {
  en_US: 'en',
  it_IT: 'it'
}
export const Termini = {
  en_US: {
    azienda: 'company',
    prodotti: 'products',
    tutti_prodotti: 'all products',
    tasto_ricerca: 'Search for model',
    eventi: 'fair and events',
    accoglienza: 'Our..',
    scopri: 'discover',
    prodotti: 'products',
    macchine: 'machines',
    pitagora_desc: 'ENG Il software di programmazione avanzato con simulazione grafica 3D, elaborato da Bacci MAcchine per Centri di Lavoro CNC da 5 a 13 assi. ',
    tecnologia: 'tecnology',
    correlati: 'realted products'

  },
  it_IT: {
    azienda: 'azienda',
    prodotti: 'prodotti',
    tutti_prodotti: 'tutti i prodotti',
    tasto_ricerca: 'Ricerca Modello',
    eventi: 'eventi e fiere',
    accoglienza: "il nostro personale è pronto ad accoglierti",
    scopri: 'scopri',
    prodotti: 'prodotti',
    macchine: 'macchine',
    pitagora_desc: 'Il software di programmazione avanzato con simulazione grafica 3D, elaborato da Bacci MAcchine per Centri di Lavoro CNC da 5 a 13 assi. ',
    tecnologia: 'tecnologia',
    correlati: 'prodotti correlati'
  },
}